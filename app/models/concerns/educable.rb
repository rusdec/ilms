module Educable
  ...
  included do
    ...
    has_many :passed_passages, -> { where(status: Status.passed) }, class_name: 'Passage'
    ...
    has_many :in_progress_passages, -> { where(status: Status.in_progress) }, class_name: 'Passage'
    ...
    has_many :unavailable_passages, -> { where(status: Status.unavailable) }, class_name: 'Passage'
